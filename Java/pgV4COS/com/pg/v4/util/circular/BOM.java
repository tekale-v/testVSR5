//Added by DSM (Sogeti) 2018x.5 December Downtime for COS Circular Defect ID - 36756
...
import com.pg.v4.migrate.pgCountryOfSaleCalculation;
import com.pg.v4.util.circular.beans.Part;
...
public class BOM {
	...
	static String strMOSCircularValidType = pgCountryOfSaleCalculation.props.getProperty("MOS_Circular_Valid_Types");
	static String strMOSCircularParentTypes = pgCountryOfSaleCalculation.props.getProperty("MOS_Circular_Parent_Types");
	static String strProductPartTypes = new StringBuilder(pgV3Constants.TYPE_PGPACKINGMATERIAL).append(",")
			.append(pgV3Constants.TYPE_PACKAGINGMATERIALPART).append(",")
			.append(pgV3Constants.TYPE_PGONLINEPRINTINGPART).toString();
	...
	**/
    ...
    public static class Structure {
		 ...
		 * @return - Structure
		 **/
		public Structure(Context context, String objectId) {
		...
		public Structure checkCircular() throws FrameworkException {
			...
			if (null != parent) {
				logger.info("Circular checking data: " + parent.getType()+"|"+parent.getName()+"|"+parent.getRevision());	
				isCircular(parent);
			}
			return this;
		}
		...
		public BOM getInstance() {
			logger.info("Circular: " + circularExist);
			return new BOM(this);
		}
		//Added/Modified by DSM(Sogeti) 2018x.6 for Defect 38965 for MOS Circular : Starts
		/**
		...
		private MapList removeWithoutMarkStructureParts(MapList objectList) {
			...
			if (objectList!=null && !objectList.isEmpty()) {
				...
				for (Iterator iterator = objectList.iterator(); iterator.hasNext();) {
					...
					if (!parentTypeIsFound && (strMOSCircularParentTypes.contains(currentPart.getType()) || !isRequiredTypeForCircularRefrenceChecking(currentPart.getType()))) {
						parentTypeLevel=Integer.parseInt(currentPart.getLevel());
					...
					if (parentTypeIsFound && parentTypeLevel<level) {
						continue;
					}else if(parentTypeIsFound && parentTypeLevel>=level){
		...
		public void isParentExistInSubstituteBOMStructure(Part currentPart, List<Part> substitutes)
			...
			if (!circularExist) {
				for (Part substitute : substitutes) {
					...
					if (isRequiredTypeForCircularRefrenceChecking(substitute.getType())) {
			...
			if(parentList!=null && !parentList.isEmpty()) {
				...
				for (int i = parentList.size()-1; i >=0; i--) {
					...
					if(i==0) {
						...
						sw.append(strSubValue+CircularConstant.FORWARD_ARROWS.getValue());	
					}
				}
			}
		...
		private static String createSubstituteReferance(Part part){
			return part.isSubstitutePart()?part.getSubstituteForName()+"-> "+CircularConstant.SUBSTITUTE.getValue()+" ->"+part.getName():part.getName();					
		}
		/**
		 * @param sType
		 * @param sObjectId
		...
		private void checkCircularRefrence(String sType, String sObjectId, Part parent) throws FrameworkException {
			if (strMOSCircularParentTypes.contains(sType)) {
				BOM bomObj=new BOM.Structure(context, sObjectId).checkCircular(parent)
						.getInstance();
		...
		private void updateCircularHistory(Part parentPart, Part currentPart) {
			...
			strReference=new StringBuilder(CircularConstant.CIRCULAR_REFERENCE_MESSAGE.getValue()).append(strReference+CircularConstant.FORWARD_ARROWS.getValue()).append(createSubstituteReferance(currentPart)).toString();
			logger.info(strReference);
			circularExist = true;
			this.appendHistory(strReference);
			logger.info(CircularConstant.CIRCULAR_HISTORY_UPDATE.getValue());
		}
		//Added/Modified by DSM(Sogeti) 2018x.6 for Defect 38965 for MOS Circular : Ends
		/**
		 * This method fetch BOM from substitute
		 * 
		 * @return - MapList
    	...
    	**/
		...
		private List<Part> getSubstitutes(Map<?, ?> busMap,Part parent) {
			...
			if (busMap.containsKey(pgV3Constants.SELECT_EBOM_SUBSTITUTE_REL_ID)) {
				...
				//Added by DSM(Sogeti) 2018x.6 for Defect 38965 for MOS Circular : Starts
				StringList substituteIsMarkForCOSCal = getEBOMSubstituteFromMap(busMap,
						pgV3Constants.SELECT_EBOM_SUBSTITUTE_ATTRIBUTE_PGCOSCALCULATE);
				StringList substituteIsMarkForCOSWIP = getEBOMSubstituteFromMap(busMap,
						pgV3Constants.SELECT_EBOM_SUBSTITUTE_ATTRIBUTE_PGCOSWIP);
				//Added by DSM(Sogeti) 2018x.6 for Defect 38965 for MOS Circular : Ends
				...
				for (int i = 0; i < substituteIDs.size(); i++) {
					...
					//Added by DSM(Sogeti) 2018x.6 for Defect 38965 for MOS Circular : Starts
					beanMap.put(pgV3Constants.SELECT_EBOM_SUBSTITUTE_ATTRIBUTE_PGCOSCALCULATE,
							substituteIsMarkForCOSCal.get(i));
					beanMap.put(pgV3Constants.SELECT_EBOM_SUBSTITUTE_ATTRIBUTE_PGCOSWIP,
							substituteIsMarkForCOSWIP.get(i));
					//Added by DSM(Sogeti) 2018x.6 for Defect 38965 for MOS Circular : Ends
		...
		private Part getPartBean(String objectId) {
			...
			try {
				...
				info.put(DomainConstants.SELECT_LEVEL, CircularConstant.LEVEL_ZERO.getValue());
		...
		private StringList getBusBasicSelects() {
			...
			selectList.add(pgV3Constants.SELECT_ATTRIBUTE_PGCOSCALCULATE);
			selectList.add(pgV3Constants.SELECT_ATTRIBUTE_PGCOSWIP);
			return selectList;
		}
		...
		private StringList getRelSubstituteSelects() {
			...
			selectList.add(pgV3Constants.SELECT_EBOM_SUBSTITUTE_ATTRIBUTE_PGCOSCALCULATE);
			selectList.add(pgV3Constants.SELECT_EBOM_SUBSTITUTE_ATTRIBUTE_PGCOSWIP);
			return selectList;
		}
		//Added by DSM(Sogeti) 2018x.6 for Defect 38965 for MOS Circular : Starts
		/**
