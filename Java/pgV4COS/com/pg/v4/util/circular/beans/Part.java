...
public class Part {
    ...
    boolean isMarkForCOSCal;
    boolean isSetProdcutPresent;
    boolean isMarkForCOSWIP;
    ...
    public Part(Map<?, ?> busMap) {
    	...
    	//Added by DSM(Sogeti) 2018x.6 for Defect 38965 for MOS Circular : Starts
        if (busMap.containsKey(pgV3Constants.SELECT_ATTRIBUTE_PGCOSCALCULATE)) {
        	 this.isMarkForCOSCal =busMap.get(pgV3Constants.SELECT_ATTRIBUTE_PGCOSCALCULATE).toString().equalsIgnoreCase("true")?true:false;
        	 this.isMarkForCOSWIP=busMap.get(pgV3Constants.SELECT_ATTRIBUTE_PGCOSWIP).toString().equalsIgnoreCase("true")?true:false;
		}else {
			 this.isMarkForCOSCal =busMap.get(pgV3Constants.SELECT_EBOM_SUBSTITUTE_ATTRIBUTE_PGCOSCALCULATE).toString().equalsIgnoreCase("true")?true:false;
			 this.isMarkForCOSWIP =busMap.get(pgV3Constants.SELECT_EBOM_SUBSTITUTE_ATTRIBUTE_PGCOSWIP).toString().equalsIgnoreCase("true")?true:false;
		}
    	//Added by DSM(Sogeti) 2018x.6 for Defect 38965 for MOS Circular : Ends
	...
	//Added by DSM(Sogeti) 2018x.6 for Defect 38965 for MOS Circular : Starts
	/**
	 * @return the isMarkForCOSCal
	 */
	public boolean isMarkForCOSCal() {
		return isMarkForCOSCal;
	}
	/**
	 * @param isMarkForCOSCal the isMarkForCOSCal to set
	 */
	public void setMarkForCOSCal(boolean isMarkForCOSCal) {
		this.isMarkForCOSCal = isMarkForCOSCal;
	}
	/**
	...
	/**
	 * @return the isMarkForCOSWIP
	 */
	public boolean isMarkForCOSWIP() {
		return isMarkForCOSWIP;
	}
	/**
	 * @param isMarkForCOSWIP the isMarkForCOSWIP to set
	 */
	public void setMarkForCOSWIP(boolean isMarkForCOSWIP) {
		this.isMarkForCOSWIP = isMarkForCOSWIP;
	}
	/**
