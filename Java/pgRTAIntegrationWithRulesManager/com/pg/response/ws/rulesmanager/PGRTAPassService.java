...
@Path("/rulesManager")
public class PGRTAPassService  extends RestService {
	...
	private void createJobObject(Context context,String sJSON,String sOwner) throws FrameworkException{
		...
		job.setAttributeValue(context,DomainConstants.ATTRIBUTE_TITLE,"RulesManager_CreateCERequest");
		job.setDescription(context,sJSON);
		logger.info("Exit method createJobObject()");
	}
	
	  @POST
	  @Path("/createCopyElements")
	  @Consumes({"application/json"})
	  @Produces({"application/json"})
	  public Response createCopyElements(@javax.ws.rs.core.Context HttpServletRequest paramHttpServletRequest, RequestDataFromRulesManager reqDataFromRM)throws Exception {
