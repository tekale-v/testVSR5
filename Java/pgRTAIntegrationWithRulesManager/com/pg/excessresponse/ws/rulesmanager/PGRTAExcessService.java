...
import com.pg.response.ws.rulesmanager.PGResponseData;
...
@Path("/rulesManager")
public class PGRTAExcessService  extends RestService {
	private static final Logger logger = Logger.getLogger("com.pg.excessresponse.ws.rulesmanager.PGRTAEXCESService");
	private static final String MSG_200 = "CE Connection request submitted successfully and is in progress. Once processed, RTA will send the response back.";
	...
	private void createJobObject(Context context,String sJSON,String sOwner) throws FrameworkException{
		...
		job.setAttributeValue(context,DomainConstants.ATTRIBUTE_TITLE,"RulesManager_ConnectCERequest");
		job.setDescription(context,sJSON);
		logger.info("Exit method createJobObject()");
	}
	  @POST
	  @Path("/connectCopyElements")
	  @Consumes({"application/json"})
	  @Produces({"application/json"})
	  public Response connectCopyElements(@javax.ws.rs.core.Context HttpServletRequest paramHttpServletRequest, RequestDataFromRulesManager reqDataFromRM)throws Exception {
	    logger.info("\n Enter connectCopyElements() starts");
	    Context context = null;
				...
				String sWbServiceUser = EnoviaResourceBundle.getProperty(context, "emxCPN.AAL.applicationUser");
	    	  	...
	    	  	if(BusinessUtil.isNotNullOrEmpty(strPersonId)) {
	    	  		...
	    	  		if(DomainConstants.STATE_PERSON_ACTIVE.equals(domObj.getInfo(context, DomainConstants.SELECT_CURRENT))) {
	    				...
	    				List<com.pg.excessresponse.ws.rulesmanager.POA> poaDataFromRM = reqDataFromRM.getPoa();
	    ...
	    logger.info("\n Enter connectCopyElements() Ends.");
