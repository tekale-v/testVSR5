/*
 **   DeviceProductPart.java
 **   Description - Introduced as part of Battery Data Load - 18x.6
 **   About - Class to perform battery data load for type DPP.
 **
 */
...
import com.pg.dsm.upload.battery.models.DeviceProductPartBean;
...
public class DeviceProductPart implements Product {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    BatteryProduct batteryProductConfig;
    Context context;
    BatteryPropertyConfig batteryPropertyConfig;

    public DeviceProductPart(Context context, BatteryPropertyConfig batteryPropertyConfig, BatteryProduct batteryProductConfig) {
    ...
    public Map<String, ProductPartBean> getProductPartBeanMap(MapList objectList) {
        ...
        for (Object eachObj : objectList) {
            tempMap = (Map<?, ?>) eachObj;
            dataMap.put(generateNameRevisionKey(tempMap), new DeviceProductPartBean(context, tempMap));
        }
    ...
    public void performValidations(List<BatteryProduct> batteryProductList) {
        try {
            Class<?> classDeviceProductPartVerifier = Class.forName(DeviceProductPartVerifier.class.getName());
            Class<?>[] classDeviceProductPartVerifierConstructorArgs = new Class[3];
            classDeviceProductPartVerifierConstructorArgs[0] = BatteryProduct.class;
            classDeviceProductPartVerifierConstructorArgs[1] = BatteryProductFeature.class;
            classDeviceProductPartVerifierConstructorArgs[2] = BatteryPropertyConfig.class;
            ...
            while (batteryProductIterator.hasNext()) {
                ...
                if (batteryProduct.isBeanExist()) {
                    batteryProductErrorMessageList.addAll(performValidation(batteryProduct, batteryProductFeatures, classDeviceProductPartVerifier, classDeviceProductPartVerifierConstructorArgs));
                } else {
     ...
     * @since DSM 2018x.6
     */
    private List<String> performValidation(BatteryProduct batteryProduct, List<BatteryProductFeature> batteryProductFeatures, Class<?> classDeviceProductPartVerifier, Class<?>[] classDeviceProductPartVerifierConstructorArgs) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException {
        List<String> batteryProductErrorMessageList = new ArrayList<>();
        String validationMethod;
        DeviceProductPartVerifier deviceProductPartVerifier;
        ...
        for (BatteryProductFeature batteryProductFeature : batteryProductFeatures) {
            ...
            if (UIUtil.isNotNullAndNotEmpty(validationMethod)) {
                deviceProductPartVerifier = (DeviceProductPartVerifier) classDeviceProductPartVerifier.getDeclaredConstructor(classDeviceProductPartVerifierConstructorArgs).newInstance(batteryProduct, batteryProductFeature, batteryPropertyConfig);
                validationMethodObj = classDeviceProductPartVerifier.getDeclaredMethod(validationMethod);
                batteryProductFeatureErrorMessageList = (List<String>) validationMethodObj.invoke(deviceProductPartVerifier);
        ...
        try {
            ...
            for (BatteryProduct batteryProduct : batteryProductList) {
                ...
                if (batteryProduct.isBeanExist()) {
                    ...
                    if(pgV3Constants.TYPE_DEVICEPRODUCTPART.equalsIgnoreCase(batteryProduct.getProductPartBean().getType()) 
                    			&& pgV3Constants.STATE_RELEASE.equalsIgnoreCase(batteryProduct.getProductPartBean().getCurrentState()))	{
                    ...
                    logger.info(String.format("Record updated successfully: %s | %s",batteryObject.getName(),batteryObject.getRevision()));
    ...
    public StringList getBusAttributeSelects() {
        ...
        busSelects.addElement(BatteryConstants.Attribute.POWER_SOURCE.getAttributeSelect(context));
        busSelects.addElement(BatteryConstants.Attribute.BATTERY_TYPE.getAttributeSelect(context));
        busSelects.addElement(BatteryConstants.Attribute.BATTERIES_SHIPPED_INSIDE_DEVICE.getAttributeSelect(context));
        busSelects.addElement(BatteryConstants.Attribute.NUMBER_OF_BATTERIES_REQUIRED.getAttributeSelect(context));
        busSelects.addElement(BatteryConstants.Attribute.NET_WEIGHT.getAttributeSelect(context));
        return busSelects;
    }
