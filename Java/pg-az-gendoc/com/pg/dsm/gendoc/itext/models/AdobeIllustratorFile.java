/*
 **   IllustratorArtworkFile.java
 **   Description - Introduced as part of Cloud GenDoc - 18x.6 July CW for Defect 43662
 **   About - Implementation class to convert Illustrator Artwork file to pdf.
 **
 */
...
public class AdobeIllustratorFile implements PDFDocument {
	...
	public AdobeIllustratorFile(Document document, String localDownloadPath) {
		this.document = document;
		this.localDownloadPath = localDownloadPath;
	}

	public AdobeIllustratorFile() {
	}
	...
	public PDFDocument getPDFDocument() throws InvocationTargetException, IllegalAccessException, IOException, DocumentException {
		...
		PDFDocument pdfDocument = new AdobeIllustratorFile();
		...
		// get the instance of pdf writer.
		...
		try (PdfDocument writer = new PdfDocument(new PdfWriter(downloadPathBuilder.toString()))) {
			PdfDocument reader = new PdfDocument(new PdfReader(inWorkPathBuilder.toString()));
			reader.copyPagesTo(1, reader.getNumberOfPages(), writer);
			reader.close();
			com.itextpdf.layout.Document iTextDocument = new com.itextpdf.layout.Document(writer);
			iTextDocument.close();
		}
