...
public class RawMaterialUserPreferenceTemplateService {
    boolean applied;
    String error;

    private RawMaterialUserPreferenceTemplateService(Apply apply) {
    ...
    public static class Apply {
        ...
        public RawMaterialUserPreferenceTemplateService now(DomainObject domainObject, String objectOid, String symbolicType, String uptOid) throws Exception {
            try {
                applyNow(domainObject, objectOid, symbolicType, uptOid);
                this.applied = Boolean.TRUE;
            } catch (Exception e) {
                this.applied = Boolean.FALSE;
                logger.log(Level.WARNING, "Error Applying Raw Material User Preference Template:" + e);
                throw e;
            }
            return new RawMaterialUserPreferenceTemplateService(this);
        }
        ...
        private void applyNow(DomainObject domainObject, String objectOid, String symbolicType, String uptOid) throws Exception {
            try {
                ...
                RawMaterialUserTemplate rawMaterialUserTemplate = userPreferenceUtil.getRawMaterialUserTemplate(context, uptOid);
                Map<String, String> attributeMap = new HashMap<>();
                String partType = rawMaterialUserTemplate.getType();
                if (UIUtil.isNotNullAndNotEmpty(partType)) {
                    ...
                    if (UIUtil.isNotNullAndNotEmpty(partType) && symbolicType.equalsIgnoreCase(partType)) { // if incoming type is equal to packaging preference - part type.}
                        ...
                        if (userPreferenceUtil.isPreferencePhaseMatchWithPhaseAttribute(this.context, domainObject, rawMaterialUserTemplate.getPhase())) {
                            String manufacturingStatus = rawMaterialUserTemplate.getMfgStatus();
                ...
                this.applyReportedFunction(userPreferenceUtil, domainObject, symbolicType, rawMaterialUserTemplate.getReportedFunction());

                // apply structure release criteria (attribute)
                String releaseCriteria = rawMaterialUserTemplate.getReleaseCriteria();
                ...
                String classValue = rawMaterialUserTemplate.getClassType();
                ...
                // apply business area
                this.applyBusinessArea(userPreferenceUtil, domainObject, symbolicType, rawMaterialUserTemplate.getBusinessArea());// business area physical id is stored on preference.

                // apply product category platform
                this.applyProductCategoryPlatform(userPreferenceUtil, domainObject, symbolicType, rawMaterialUserTemplate.getCategoryPlatform()); // Product Category Platform physical id is stored on preference.

                // apply function
                this.applyMaterialFunctionGlobal(userPreferenceUtil, domainObject, symbolicType, rawMaterialUserTemplate.getMaterialFunction());// Material Function object id is stored.
