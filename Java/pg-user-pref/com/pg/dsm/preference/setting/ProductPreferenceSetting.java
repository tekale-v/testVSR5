...
public class ProductPreferenceSetting {
    boolean applied;

    private ProductPreferenceSetting(Apply apply) {
    ...
    public static class Apply {
        ...
        public ProductPreferenceSetting now(DomainObject domainObject, String objectOid, String symbolicType) throws Exception {
            try {
            ...
            return new ProductPreferenceSetting(this);
        }
        ...
        private void applyNow(DomainObject domainObject, String objectOid, String symbolicType) throws Exception {
            try {
                ...
                ProductPreference productPreference = new ProductPreference(this.context);
                ...
                if (UIUtil.isNotNullAndNotEmpty(partType)) {
                    ...
                    if (isFormulation) {
                        if (pgV3Constants.TYPE_FORMULATIONPART.equals(incomingType)) {
                            this.applyReportedFunction(userPreferenceUtil, domainObject, symbolicType, productPreference.getReportedFunction());
                        }
                    } else {
                        if (UIUtil.isNotNullAndNotEmpty(partType) && symbolicType.equalsIgnoreCase(partType)) { // if incoming type is equal to product preference - part type.}
                            ...
                            if (userPreferenceUtil.isPreferencePhaseMatchWithPhaseAttribute(this.context, domainObject, productPreference.getPhase())) {
                ...
                String releaseCriteria = productPreference.getReleaseCriteria();
                ...
                String classValue = productPreference.getClassValue();
                ...
                String productComplianceRequired = productPreference.getProductComplianceRequired();
                ...
                this.applyBusinessArea(userPreferenceUtil, domainObject, symbolicType, productPreference.getBusinessAreaID());// business area physical id is stored on preference.

                // apply product category platform (relationship)
                this.applyProductCategoryPlatform(userPreferenceUtil, domainObject, symbolicType, productPreference.getProductCategoryPlatformID()); // Product Category Platform physical id is stored on preference.
