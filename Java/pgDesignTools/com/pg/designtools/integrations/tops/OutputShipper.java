...
public class OutputShipper implements IMappableComponent {
	@XStreamAlias("dims")
	private Dims dims;
	@XStreamAlias("info")
	...
	AttributeList mappingRelAttributes = new AttributeList();
	String sLenUnits;
	...
	/**
	 * @return an AttributeList of mapping attributes defined in
	 *         processMappingAttributes and called by mapAttributes(node)
	 */
	...
	private void processGeometryBodyInterfaces(){
		...
		if(UIUtil.isNotNullAndNotEmpty(sGeometryBodyInterfaceName)) {
			switch (sGeometryBodyInterfaceName) {
				 ...
				 case "pgTOPSDrum" :
					 mappingAttributes.addElement(new Attribute(new AttributeType(DataConstants.ATTRIBUTE_PG_DIAMETER),objectCPD.convertDimensionToMM(dims.getHgt(),sLenUnits)));
					 break;
				 case "pgTOPSBucketRound" : 
	...
	public String getIPackq()
	{
		if(info!=null)
			return info.getIpacks();
		else
			return "1";	
	}
