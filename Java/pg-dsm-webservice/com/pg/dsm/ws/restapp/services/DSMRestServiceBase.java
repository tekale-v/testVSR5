...
abstract class DSMRestServiceBase extends RestService
{
...
protected void closeSession(WebServiceContext paramWebServiceContext, LogContext paramLogContext) {
    try {
      ...
      Logger.getLogger(DSMRestServiceBase.class.getName()).log(Level.WARNING, "Got exception: {0}", localPLMxModelerException.toString());
    }
    try
    {
      ...
      if (null != localContext) {
    	...
    	Logger.getLogger(DSMRestServiceBase.class.getName()).log(Level.WARNING, "Got exception: {0}", localMatrixException.toString());
    }
  }
}